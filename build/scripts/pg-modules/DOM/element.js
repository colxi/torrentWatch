            ...
            closest: function closest(selector) {
                ...
                for (; elem && elem !== document && elem.nodeType === 1; elem = elem.parentNode) {
                    switch (firstChar) {
                        ...
                        case '.':
                            if (supports) if (elem.classList.contains(selector.substr(1))) return elem;else if (new RegExp('(^|\\s)' + selector.substr(1) + '(\\s|$)').test(elem.className)) return elem;
                            break;
                        case '#':
                        ...
                        case '[':
                            if (elem.hasAttribute(attribute[0])) {
                                if (value && elem.getAttribute(attribute[0]) === attribute[1]) return elem;else return elem;
                            }
                            break;
                        default:
