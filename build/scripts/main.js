...
(function () {
   ...
   * @return {[type]}            [description]
   */
		loadController: function loadController(controller) {
			...
			return new Promise(function (resolve, reject) {
				...
				pg.feeds.reduce(function (sequence) {
					return sequence.then(function () {
						...
						if (result) pg.log('pg.updateAllFeeds(): Feed #' + (currentFeed + 1) + ' ' + pg.feeds[currentFeed].name + ' UPDATED (' + result + ')');else pg.log('pg.updateAllFeeds(): Feed #' + (currentFeed + 1) + ' ' + pg.feeds[currentFeed].name + ' FAILED (' + result + ')');
					});
				}, Promise.resolve());
			});
		},
